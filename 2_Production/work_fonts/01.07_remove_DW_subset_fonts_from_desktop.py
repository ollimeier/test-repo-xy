...
def run_post_script(path):
    for filename in os.listdir(path):
        ...
        if suffix.lower() in font_suffixes:
            ...
            remove = False
            if 'LatinBasicLetters' in filename:
                remove = True
            if 'LatinBasicSymbols' in filename:
                remove = True
            if 'Greek' in filename:
                remove = True
            if 'Cyrillic' in filename:
                remove = True
            if 'Arabic' in filename:
                remove = True
            if 'LatinExtended' in filename:
                remove = True

            if remove:
                print('Removing DW font file: ', filename)
                os.remove(full_path)
